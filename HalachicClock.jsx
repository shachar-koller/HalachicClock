...
export default function HalachicClock() {
  ...
  const shellClass = clockData.isDayMode
    ? "bg-sky-50 border-sky-200 text-slate-800"
    : "bg-slate-900 border-slate-700 text-slate-100";

  if (loadError) {
    return (
      <div className="mx-auto w-full max-w-md rounded-2xl border border-red-200 bg-red-50 p-4 text-red-700">
        {loadError}
      </div>
    );
  }
  ...
  return (
    <div className={`mx-auto w-full max-w-md rounded-2xl border p-5 shadow-lg transition-colors ${shellClass}`}>
      <h2 className="mb-4 text-center text-xl font-semibold">Halachic Clock</h2>

      <div className="mb-4 flex justify-center">
        <canvas ref={canvasRef} className="h-80 w-80 max-w-full" />
      </div>

      <div className="space-y-1 text-sm">
        <p>
          <span className="font-semibold">Halachic time:</span> {formatHalachicTime(clockData.halachicHour)}
        </p>
        <p>
          <span className="font-semibold">Sunrise:</span> {formatCivilTime(clockData.sunrise, false)}
          {"  "}
          <span className="font-semibold">Sunset:</span> {formatCivilTime(clockData.sunset, false)}
        </p>
        <p>
          <span className="font-semibold">Civil time:</span> {formatCivilTime(clockData.now)}
        </p>
        <p>
          <span className="font-semibold">Location:</span> {location.label}
        </p>
        <p className={clockData.isDayMode ? "text-slate-600" : "text-slate-300"}>
